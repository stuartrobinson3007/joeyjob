...
import { useDebouncedCallback } from "taali/frontend";
...
function DataTableFilterMenuImpl<TData>({
  table,
  ...
  // Use TanStack columnFilters state instead of external URL state
  ...
  const setFilters = React.useCallback((newFilters: ExtendedColumnFilter<TData>[]) => {
    // Convert ExtendedColumnFilter back to TanStack format
    const tanstackFilters = newFilters.map(filter => ({
      id: filter.id,
      value: {
      ...
      >
        ...
        >
          <SelectTrigger
            ...
            className="h-8 rounded-none border-r-0 px-2.5 lowercase [&[data-size]]:h-8 [&_svg]:hidden hover:bg-accent hover:text-accent-foreground"
          >
        ...
        <Button
          ...
          size="sm"
          className={cn(
            "h-full rounded-none rounded-r-md border px-1.5 font-normal dark:bg-input/30",
...
export const DataTableFilterMenu = React.memo(DataTableFilterMenuImpl);
