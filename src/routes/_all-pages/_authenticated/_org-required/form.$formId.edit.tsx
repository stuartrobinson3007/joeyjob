...
import { SubscriptionRequiredDialog } from '@/features/booking/components/subscription-required-dialog'
import {
...
export const Route = createFileRoute('/_all-pages/_authenticated/_org-required/form/$formId/edit')({
...
function FormEditorPage() {
  ...
  const [autosaveState, autosaveActions] = useUnifiedAutosave(
    ...
    handleSave,
    {
      ...
      isFormEnabled: optimisticIsActive !== null ? optimisticIsActive : form?.isActive || false
    }
  );
  ...
  const toggleActiveMutation = useMutation({
    ...
    onError: useCallback((error: any, isActive: boolean) => {
      ...
      if (error?.code === ERROR_CODES.BIZ_SUBSCRIPTION_REQUIRED ||
          error?.details?.requiresSubscription) {
        // Show subscription dialog instead of error toast
    ...
    <>
      ...
      >
        <FormEditorLayout
          ...
          validationResult={autosaveState.validationResult}
          canSave={autosaveState.canSave}
          saveBlockedReason={autosaveState.saveBlockedReason}
        />
      </FormEditorDataProvider>
