...
const formatFileSize = (bytes: number, t: (key: string) => string) => {
  if (bytes === 0) return `0 ${t('fileUpload.fileSizes.bytes')}`
  const k = 1024
  const sizes = [
    t('fileUpload.fileSizes.bytes'),
    t('fileUpload.fileSizes.kb'),
    t('fileUpload.fileSizes.mb'),
  ]
    ...
    (files: FileList | null) => {
      ...
      filesToUpload.forEach(file => {
        if (!acceptedFileTypes.includes(file.type)) {
          handleUploadError(
            t('fileUpload.invalidFileType', { types: acceptedFileTypes.join(', ') })
          )
          return
        }

        if (file.size > maxSizeInMB * 1024 * 1024) {
          handleUploadError(t('fileUpload.fileSizeLimit', { maxSize: maxSizeInMB }))
          return
        }

        if (isFileDuplicate(file)) {
          handleUploadError(t('fileUpload.fileDuplicate', { fileName: file.name }))
          return
        }
  ...
  const handleDeleteFile = useCallback(
    (fileId: number) => {
      ...
      if (fileToDelete) {
        ...
        handleDelete(fileToDelete as unknown as File)
      }
    },
    [uploadedFiles, handleDelete]
  )
    ...
    <div className="text-center">
      ...
      <div className="mt-4">
        ...
        <p className="mt-2 text-xs sm:text-sm text-muted-foreground">
          {t('fileUpload.dropFiles', { plural: multiple ? 's' : '' })}
        </p>
    ...
    >
      <div className="flex items-center justify-between p-2 sm:p-3 bg-muted rounded-lg overflow-hidden">
        <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
          <div
            className={`shrink-0 flex size-6 sm:size-8 p-1 sm:p-2 items-center justify-center rounded bg-background`}
          >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground truncate">
              {formatFileSize(currentUpload!.file.size, t)}
            </p>
          </div>
        </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <span className="text-xs sm:text-sm text-muted-foreground">
                ...
                {uploadedFiles.length === 1
                  ? t('fileUpload.fileCount.single')
                  : t('fileUpload.fileCount.multiple')}
              </span>
            </CardHeader>
